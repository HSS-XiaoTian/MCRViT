...
class Morlet_2(nn.Module):
    def __init__(self, size):
        super(Morlet_2, self).__init__()
        self.a = nn.Parameter(torch.ones(size))
        self.b = nn.Parameter(torch.zeros(size))

    def wavelet_basis(self, t):
        freq = 1.90
        return (-1 * torch.exp(-t ** 2 / 2) * (
                ((t ** 2 - freq ** 2 - 1) * torch.cos(freq * t)) + 2 * freq * t * torch.sin(
            freq * t)))

    def forward(self, s):
        t = (s - self.b) / self.a
        return self.wavelet_basis(t)
...
class MCRViT(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100):
        ...
        self.lstm = nn.LSTM(input_size=512, hidden_size=1024, num_layers=2, batch_first=True, dropout=0.5)
...
def mk_MCRViT(num_classes):
    return MCRViT(BasicBlock, [2, 2, 2, 2], num_classes)
